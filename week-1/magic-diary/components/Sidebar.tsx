...
export function Sidebar({ entries, selectedEntryId, onSelectEntry, onNewEntry }: SidebarProps) {
  ...
  const availableMonths = useMemo(() => {
    ...
    entries.forEach(e => {
      ...
      if (!map.has(key)) {
        map.set(key, { label: `${MONTHS_SHORT[d.getMonth()]} ${d.getFullYear()}` })
      }
    })
      ...
      {availableMonths.length > 1 && (
        <div className="flex gap-1 overflow-x-auto px-2 pb-2 scrollbar-hide">
          <button
            ...
            className={[
              'flex-shrink-0 px-2.5 py-1 rounded-full tracking-wide transition-all',
              monthFilter === null
                ? 'bg-[#C9993F]/30 text-[#F0C96A]'
                : 'bg-[rgba(255,255,255,0.05)] text-[#9A7A5A] hover:bg-[#C9993F]/15',
            ].join(' ')}
          >
            Wszystkie
          </button>
          {availableMonths.map(({ key, label }) => (
            <button
              ...
              style={{ fontFamily: "'Cinzel', serif", fontSize: 10 }}
              className={[
                'flex-shrink-0 px-2.5 py-1 rounded-full tracking-wide transition-all whitespace-nowrap',
                monthFilter === key
                  ? 'bg-[#C9993F]/30 text-[#F0C96A]'
                  : 'bg-[rgba(255,255,255,0.05)] text-[#9A7A5A] hover:bg-[#C9993F]/15',
              ].join(' ')}
